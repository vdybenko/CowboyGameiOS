...
ServerBrowser ServerBrowserNew(const gsi_char *queryForGamename, 
							   ...
							   const gsi_char *queryFromKey, int queryFromVersion, 
							   int maxConcUpdates, int queryVersion, 
							   SBBool lanBrowse, ServerBrowserCallback callback, 
							   void *instance);
...
//		This should only be called based on a user input trigger (e.g. a 'Refresh' button).
//		It should never be called on a timer.
// See Also
...
//		<p>
//		A server update is only performed when a user selects a specific server.
//		Full updates for all servers on the list are not requested automatically
//		(and ServerBrowserAuxUpdateServer() is not called from within the ServerBrowserCallback).
// See Also
...
//		<p>
//		This should only be initiated when a player has selected a server to connect to.
//		It should never be used when a player is browsing for a server.
// Remarks
