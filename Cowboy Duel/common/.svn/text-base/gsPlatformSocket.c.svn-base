...
// handle errors after performing selects.
		...
		////////////////////////
		if((theReadFlag != NULL)&&(theSocket != INVALID_SOCKET))
		{
			FD_ZERO(&aReadSet);
			FD_SET(theSocket,&aReadSet);
			aReadFds = &aReadSet;
		}
		if((theWriteFlag != NULL)&&(theSocket != INVALID_SOCKET))
		{
			FD_ZERO(&aWriteSet);
			FD_SET(theSocket, &aWriteSet);
			aWriteFds = &aWriteSet;
		}
		if((theExceptFlag != NULL)&&(theSocket != INVALID_SOCKET))
		{
			FD_ZERO(&aExceptSet);
