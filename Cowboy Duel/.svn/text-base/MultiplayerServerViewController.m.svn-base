//
//  MultiplayerServerViewController.m
//  Cowboy Duel 1
//
...
int tryread(SOCKET s)
{
    ...
    {
        // printf("recvfrom %ld", recvfrom(s, receiveBuffer, sizeof(receiveBuffer) - 1, 0, (struct sockaddr *)&saddr, &saddrlen));
        len = recvfrom(s, receiveBuffer, sizeof(receiveBuffer) - 1, 0, (struct sockaddr *)&saddr, &saddrlen);
        
        if (len < 0)
        {
            len = GOAGetLastError(s);
            // printf("|Got recv error: %ld\n", len);
...
void keylist_callback(qr2_key_type keytype, qr2_keybuffer_t keybuffer, void *userdata)
{
	...
	switch (keytype)
	{
        case key_server:
            ...
            qr2_keybuffer_add(keybuffer, APP_VERSION_KEY); //a custom key
...
static void init_game(void)
{
	...
	_tcscpy(gamedata.hostname,_T(hostname));
...
void DoGameStuff(gsi_time totalTime)
{
	...
	if (!stateChanged && totalTime > 30000) {
		...
		_tcscpy(gamedata.rankingson,_T("gmtmap2"));
		qr2_send_statechanged(NULL);
		stateChanged = 1;
	}
}

@interface MultiplayerServerViewController (PrivateMethods)

-(void) sendCycle:(void *) data packetID:(int)packetID ofLength:(int)length;

@end

@implementation MultiplayerServerViewController
@synthesize gameCenterViewController, isRunServer, neadRestart;
@synthesize serverNameGlobal;
@synthesize pingStart;
gameInfo gameStat;
...
- (id)initWithServerName:(NSString *)serverName
{
    ...
    if (self) {
        ...
        serverNameGlobal = serverName;
        
        //[self shutDownServer];
        if (!neadRestart) [self performSelectorInBackground:@selector(createServer) withObject:self]; 
        else [self shutDownServer];
...
-(void)createServer
{
    gameCenterViewController.multiplayerViewController = self;
    pingStart = NO;
    hostname = [serverNameGlobal cStringUsingEncoding:NSUTF8StringEncoding];
    isRunServer = YES;
    /* qr2_init parameters */
    ...
    qr2_register_key(APP_VERSION_KEY, _T("app_version")    );
    ...
    while (isRunServer)
    {
        ...
        //        if ((connected) && (current_time() - aStartTime > 1000))
        //        {
...
-(void)receivePacket
{
    ...
    int *pIntData = (int *)&receiveBuffer[0];
    int packetID = pIntData[0];
    printf("receivePacketID %d\n", packetID);
    
    if ((packetID != NETWORK_PING) && (packetID != NETWORK_RESPONSE) && (lastReceivedPacket == packetID) && (packetID != NETWORK_ACCEL_STATE)) {
        printf("MultiplayerServerViewController Packet is repeated!\n");
        [self sendResponse];
        return;
    }
    
    if ((packetID != NETWORK_PING) && (packetID != NETWORK_RESPONSE)) lastReceivedPacket = packetID;
    
    if (packetID == NETWORK_RESPONSE) neadSend = NO;
    if (packetID == NETWORK_PING) {
        printf("server send ping response!\n");
        gameInfo *gs2 = &gameStat;
        lastPingTime = current_time();
        [self sendCycle:gs2 packetID:NETWORK_PING_RESPONSE ofLength:(sizeof(gameInfo)+10)];
    }
    else
    {
        
        if ((!pingStart) || ((pingStart) && (packetID != NETWORK_TIME))) {
            NSData *packet = [NSData dataWithBytes: &receiveBuffer length: (sizeof(gameInfo) + 10)];
            [gameCenterViewController receiveData:packet];
        }
        
    }
    if (packetID == NETWORK_TIME) {
        pingTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(pingTime) userInfo:nil repeats:YES];
        pingStart = YES;
    }
}
...
-(void) sendDataData:(void *) data packetID:(int)packetID ofLength:(int)length{
    neadSend = YES;
    //if (packetID == NETWORK_START_DUEL_FALSE) connected = 0;
    ...
    while (neadSend) {
        ...
        if (current_time() - lastsendtime > 500)
        {
            ...
            if (tryCount > 10) {
                ...
                //[self performSelectorOnMainThread:@selector(navigationPopToRoot) withObject:nil waitUntilDone:YES];
                [self performSelectorOnMainThread:@selector(messageShow) withObject:nil waitUntilDone:YES];
//                self.isRunServer = NO;
//                if (sock != INVALID_SOCKET)
//                    closesocket(sock);
...
//                SocketShutDown();                    
        ...
        if (gotData) [self performSelectorOnMainThread:@selector(receivePacket) withObject:nil waitUntilDone:YES];;
...
-(void) sendCycle:(void *) data packetID:(int)packetID ofLength:(int)length
{
	...
	if(length < (kMaxTankPacketSize - packetHeaderSize)) { // our networkPacket buffer size minus the size of the header info
        ...
        int ret = sendto(sock, networkPacket, sizeof(networkPacket), 0, (struct sockaddr *)&otheraddr, sizeof(struct sockaddr_in));
...
-(void)sendResponse
{
    ...
    int error = GOAGetLastError(sock);
//    printf("|Sending (%d:%d), remoteaddr: %s, remoteport: %d\n", ret, error, inet_ntoa(otheraddr.sin_addr), ntohs(otheraddr.sin_port));
//    printf("NETWORK_RESPONSE send\n");
}

-(void)messageShow
{
    if (!pingStart) return;
    if (isRunServer) [gameCenterViewController lostConnection];
...
-(void)messageShowOnChat
{
    ...
    //[startViewController.chatViewController setMessage:NSLocalizedString(@"NoServersHTML", @"")];
}
...
-(void)pingTime
{
    ...
    if ((current_time() - lastPingTime) > 20000)
    {
        [self performSelectorOnMainThread:@selector(messageShow) withObject:nil waitUntilDone:YES];
        [pingTimer invalidate];
    }
