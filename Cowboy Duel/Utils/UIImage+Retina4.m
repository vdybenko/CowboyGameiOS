...
+ (UIImage *)retina4ImageNamed:(NSString *)imageName {
    ...
    if (retinaAtSymbol.location != NSNotFound) {
        ...
        if ([UIScreen mainScreen].scale == 2.f && screenHeight == 568.0f) {
            ...
            NSRange rangeForRemove;
            rangeForRemove.length = 4;
            rangeForRemove.location = dot.location;
            if (dot.location != NSNotFound) 
                [imageNameMutable replaceCharactersInRange:rangeForRemove withString:@""];
    ...
    if (imagePath) {
        NSRange multi = [imageNameMutable rangeOfString:@"@2x"];
        [imageNameMutable replaceCharactersInRange:multi withString:@""];
