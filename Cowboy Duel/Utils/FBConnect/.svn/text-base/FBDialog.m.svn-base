...
- (NSURL*)generateURL:(NSString*)baseURL params:(NSDictionary*)params {
    if (params) {
        ...
        NSString* url = [[NSString alloc] initWithFormat:@"%@?%@", baseURL, query];
        return [[NSURL alloc]initWithString:url];
    } else {
        return [[NSURL alloc]initWithString:baseURL];
    }
}
    ...
    if (recipientJson) {
        ...
        if ([recipients isKindOfClass:[NSArray class]]) { 
            ...
            for (id recipient in recipients) {
                NSString *fbid = [[NSString alloc]initWithFormat:@"%@", recipient];
                [ids addObject:fbid];
            }
...
- (void)loadURL:(NSString*)url get:(NSDictionary*)getParams {
    ...
    NSMutableURLRequest* request = [[NSMutableURLRequest alloc]initWithURL:_loadingURL];
