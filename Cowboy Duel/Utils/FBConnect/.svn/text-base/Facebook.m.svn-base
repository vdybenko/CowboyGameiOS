...
- (void)invalidateSession {
    ...
    NSArray* facebookCookies = [cookies cookiesForURL:
                                [[NSURL alloc] initWithString:@"http://login.facebook.com"]];
...
- (NSString *)getOwnBaseUrl {
    return [[NSString alloc]initWithFormat:@"fb%@%@://authorize",
            _appId,
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
    ...
    if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
        if (tryFBAppAuth) {
            ...
            NSString *urlPrefix = [[NSString alloc] initWithFormat:@"%@://%@", scheme, kFBAppAuthURLPath];
...
- (FBRequest*)requestWithGraphPath:(NSString *)graphPath
    ...
    return [self requestWithGraphPath:graphPath
                            andParams:[[NSMutableDictionary alloc]init]
                        andHttpMethod:@"GET"
                          andDelegate:delegate];
}
